...
begin
  ...
  begin
    ...
    begin
      ...
      qryPostgres.Params.ParamByName('department_id').AsInteger := FieldByName('setor').AsInteger;
      qryPostgres.ExecSQL;
      Next;
    end;
    Close;
    SQL.Clear;
    SQL.Text := 'select max(id_funcionario) as m from funcionario';
    Open;
    qryPostgres.Close;
    qryPostgres.SQL.Clear;
    qryPostgres.SQL.Text := 'ALTER SEQUENCE employees_id_seq RESTART WITH :max;';
    qryPostgres.Params.ParamByName('max').AsInteger := FieldByName('m').AsInteger;
    qryPostgres.ExecSQL;
  end;
end;
