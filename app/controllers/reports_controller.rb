class ReportsController < ApplicationController
  ...
  def ficha_ponto
    ...
    report = ODFReport::Report.new("#{Rails.root.to_s}/app/reports/ficha_ponto.odt") do |r|
      ...
      @points = Point.order("date_time").where("date_time between ? and ? and employee_id = ?", params[:date_start].to_date.strftime("%Y-%m-%d"), params[:date_end].to_date.strftime("%Y-%m-%d"), employee.id)
      ...
      current_date_time =  params[:date_start].to_date
      last_date_time = params[:date_end].to_date

      while current_date_time <= last_date_time do
        achou = false
        card_points.each do |c|
          if (!c.nil?) && (c.date_time == current_date_time.strftime("%d/%m/%Y"))
            achou = true
            break
          end 
        end
